...
%edited by JH 092014: Changed OPTIONS in the extract_averages code to 0 [redundant with extract_base_loadprep_data above, causing problems with any EEG2erp preprocessing done in OPTIONS.loaddata
%
...
% Features Wishlist: place .catcodes and .static_sets into subsampling.output, and the other parameters into the subsampling.input.
    ...
    for count_catcode = 1:length(catcodes2extract),
     ...
     % end  
      ...
      if isfield(OPTIONS.subsampling, 'erp_averaging') & isequal(OPTIONS.subsampling.erp_averaging, 1),
        ...
        erp_samples(count_catcode,:).erp = extract_averages(innamebeg,erp_temp,innameend,'',0,domain,0,0,startms,endms,0,0,0,'NONE',catcodes_resamp,elecs2extract,verbose); %edited by JH 092014. Changed OPTIONS to 0 [redundant with extract_base_loadprep_data above, causing problems with any EEG2erp preprocessing done in OPTIONS.loaddata
     ...
     if catcodes_loop_success == 1, % ADDED JH 01.28 3:22
       ...
       if isfield(OPTIONS.subsampling, 'erp_averaging') & isequal(OPTIONS.subsampling.erp_averaging, 1),
         ...
         if isfield(OPTIONS.subsampling, 'cache_subject_files') & isequal(OPTIONS.subsampling.cache_subject_files, 1),
           % save sbj-level file
           %save(['./temp_sbjs' filesep subnames(main_loop_counter,:) innameend OPTIONS.subsampling.temp_subject_suffix],'erp'); % edited 05.26.16 by JH for matlab 2012a compatibility, '-v7.3');
           save(['./temp_sbjs' filesep subnames(main_loop_counter,:) innameend OPTIONS.subsampling.temp_subject_suffix],'erp', '-v7.3');
 ...
 %end 
  ...
  if isempty(outname) == 0,
    if verbose >= 1, disp(['Writing file: ' outname ]); end
    %save(outname,'erp'); % edited 05.26.16 by JH for matlab2012a compatibility, '-v7.3');
    save(outname,'erp', '-v7.3');
  end
